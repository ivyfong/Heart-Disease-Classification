...
cv.res <- cv.tree(cfit, FUN=prune.tree, method="misclass", K=5) #use cross-validation to generate the pruned tree with the FUN=prune.tree argument
cv.res$size[which.min(cv.res$dev)] #print the size (number of terminal nodes) that minimizes the misclassification error = 10
cfit.pruned <- prune.misclass(cfit, best=cv.res$size[which.min(cv.res$dev)]) #using the prune.misclass function, prune the tree with the optimal size 
